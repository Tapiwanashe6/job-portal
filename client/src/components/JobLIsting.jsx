                    ...
                    isSearched && ( searchFilter.title !== "" || searchFilter.location == "") &&  (
                        <>
                            ...
                            <div className='mb-4 text-gray-600'>
                                {searchFilter.title && (
                                    <span className='inline-flex intems-center gap-2.5 bg-blue-50 border border-blue-200 px-4 py-1.5 rounded'>
                                        {searchFilter.title}
                                        <img onClick={ e => setSearchFilter(prev => ({...prev,title:""}))} className='cursor-pointer' src={assets.cross_icon} alt="" />
                                    </span>
                                )}
                                {searchFilter.location && (
                                    <span className='ml-2 inline-flex intems-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded'>
                                        {searchFilter.location}
                    ...
                    <div className='flex items-center justify-center space-x-2 mt-10'>
                        ...
                        {Array.from({length:Math.ceil(filteredJobs.length/6)}).map((_,index)=>(
                            <a key={index} href="#job-list">
                                <button onClick={()=> setCurrentPage(index+1)} className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded ${currentPage === index + 1 ? 'bg-blue-500' : 'text-gray-5--'}`}>{index + 1}</button>
                            </a>
                        ))}
                        <a href="#job-list">
